...
const Groccery_Edit = ({ id, name, salary, Number, Salary_Information  , totalAmount , date , items}) => {
  const [doctorName, setDoctorName] = useState(totalAmount);
  const [doctorSalary, setDoctorSalary] = useState(date);
  const [doctorAddress, setDoctorAddress] = useState(items);
...
console.log(id)
  ...
  const sendData = () => {
    ...
    axios
    .post(`http://localhost:8000/api/v1/updatesGroccery/${id}`, {
      name: doctorName,
        salary: doctorSalary,
  ...
  return (
    <div>
      <Button style={{ position: "relative", right: "60px" }} onClick={handleOpen}>
        Edit
      </Button>
      <Modal
        open={open}
      ...
      >
        <Box sx={style}>
          <h1 className="text-4xl text-center font-bold">Edit Groccery Detail </h1>
