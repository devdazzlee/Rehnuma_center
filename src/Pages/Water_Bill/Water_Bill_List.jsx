...
import Water_Edit from '../../Components/Modals/Water_Bill';

const Water_Bill_List = () => {
  ...
  const getAllProducts = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/api/v1/Water-Bill`);
  ...
  const deleteData = async (id)=>{
    try {
      const response = await axios.delete(`http://localhost:8000/api/v1/customer9/${id}`)
...
const formatProjectId = (projectId) => {
  ...
  <>
     <div className='flex ' >
     <h1 className="text-4xl text-center  mb-6 m-auto">Edit  Water Bill List </h1>
...
{value.dueAmount}
                ...
                <td class=" mr-8 text-right">
                    <Water_Edit  billMonth={value.billMonth}  payableAmout={value.payableAmout} billingDate={value.billingDate} dueDate={value.dueDate}  dueAmount={value.dueAmount} id={value._id}     />
                </td>
...
export default Water_Bill_List