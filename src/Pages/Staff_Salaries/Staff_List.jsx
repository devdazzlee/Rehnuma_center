...
const Staff_List = () => {
  ...
  const getAllProducts = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/api/v1/Staff-salaries`);
  ...
  const deleteData = async (id)=>{
    try {
      const response = await axios.delete(`http://localhost:8000/api/v1/customer7/${id}`)
...
const formatProjectId = (projectId) => {
  ...
  <>
     <div className='flex ' >
     <h1 className="text-4xl text-center  mb-6 m-auto">Staff  List </h1>
...
<th scope="row" class="px-6 py-4 font-medium text-black whitespace-nowrap dark:text-white">
 ...
 scope="row" class="px-6 py-4 font-medium text-black whitespace-nowrap dark:text-white">
                    ...
                    {value.staffAddress.length > 12 ? (
        <>
            {value.staffAddress.slice(0, 12)}
            ....
        </>
    ) : (
        // Display the entire content if it's 12 characters or fewer
        value.staffAddress
    )}
  </th>
                <td class=" mr-8 text-right">
                    <StaffSalary_View   staffName={value.staffName} staffSalary={value.staffSalary}  staffNumber={value.staffSalary} staffAddress={value.staffAddress}  rentAmount={value.rentAmount}  officeAddress={value.officeAddress}   officeArea={value.officeArea}   id={value._id}    />
                </td>
                <td class=" mr-8 text-right">
                    <StaffSalary_Edit  staffName={value.staffName} staffSalary={value.staffSalary}  staffNumber={value.staffSalary} staffAddress={value.staffAddress}  rentAmount={value.rentAmount}  officeAddress={value.officeAddress}   officeArea={value.officeArea}   id={value._id}     />
                </td>
